...
class ViewIdea extends JView {
	...
	{		
		...
		if($total>10)
		{
			$pagination = new Paging($total, $cur_page, 10, "index.php?option=com_intosuggest&controller=idea&task=topIdea&forum=".$forum_id."&format=raw");
	...
	function dispHotIdea($tmp = null){
		...
		if($total>10)
		{
			$pagination = new Paging($total, $cur_page, 10, "index.php?option=com_intosuggest&controller=idea&task=hotIdea&forum=".$forum_id."&format=raw");
			$pagination = $pagination->getPagination();
		}
		else 
		{
	...
	function dispNewIdea($tmp = null){
		...
		if($total>10)
		{
			$pagination = new Paging($total, $cur_page, 10, "index.php?option=com_intosuggest&controller=idea&task=newIdea&forum=".$forum_id."&format=raw");
			$pagination = $pagination->getPagination();
		}else $pagination = "";
		$this->assignRef('ideas',$ideas);
		$this->assignRef('forum_id',$forum_id);
	...
	function dispSearch($tmp = null) {	
		...
		if($model->total>10)
		{
			$pageIdea = new Paging($model->total, $page, 10, "index.php?option=com_intosuggest&controller=idea&task=search&forum=".$forum_id."&key=" .JRequest::getVar("key")."&format=raw");
		...
		switch ($box){
			...
			case 'COMMENTCOUNT':
				...
				?>			 
					...
					<a class="comment_text" href="<?php echo JRoute::_(IntoSuggestHelperRouter::addItemId('index.php?option=com_intosuggest&controller=comment&idea_id='.$idea->id))?>"> <?php echo $comment_count_text; ?></a>
				<?php 
				break;
			case 'READMORE':
				$content = htmlspecialchars_decode($idea->content); // convert quote to html tag
