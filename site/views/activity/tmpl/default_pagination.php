		...
		<td style="padding-left: 10px;padding-bottom: 15px;">
			...
			<div class="contentfooter" style="text-align: left;margin-left: 5px;">
				<div style="text-align: left;">
					...
					<!--<a href="index.php?option=com_intosuggest&controller=comment&idea_id=<?php //echo $idea->id;?>">comment</a> |-->
					<!--  -->
					<?php $db=&JFactory::getDBO();
							$query = "SELECT COUNT(*) FROM #__intosuggest_comment WHERE idea_id='".$idea->id."'";
							$db->setQuery($query);
							$rs = $db->loadResult();
							//echo $rs;
						if($rs=='0'){
					...
					?>
					<a href="index.php?option=com_intosuggest&controller=comment&idea_id=<?php echo $idea->id;?>"><?php echo JText::_("comment")?></a> |
					<?php }
					else if($rs!='0'){
						echo "<font style='color:#135CAE;font-weight:bold' id='comment_count'>".$rs."</font>";
					?>
					<a href="index.php?option=com_intosuggest&controller=comment&idea_id=<?php echo $idea->id;?>" style="font-weight:bold;"><?php echo JText::_("comment")?></a> |
					<?php }?>
					...
					<div class="small" style="float: left; margin-right: 5px;" id ="edt<?php echo $idea->id; ?>" onClick="onedit(<?php echo $idea->id; ?>)"> 
						...
						<a id="frm_Edit_<?php echo $idea->id?>" href="index.php?option=com_intosuggest&controller=idea&task=editIdea&id=<?php echo $idea->id;?>&format=raw" rel="{handler: 'iframe',size: {x: 540, y: 400}}"></a> 
					</div>						
					<?php }?>
