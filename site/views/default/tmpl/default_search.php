	...
	<div id="falcon_container">
		<div id="falcon_result">
			...
			<div class="idea-create-new" onclick='document.getElementById("frm_New").href+="&forum_id="+document.getElementById("forumId").value+"&idea_title="+document.getElementById("key_search").value; newForm("frm_New")'>
				...
				<a id="frm_New" href="index.php?option=com_intosuggest&controller=idea&task=dispNewForm&format=raw" rel="{handler: 'iframe',size: {x: 540, y: 400}}">
				</a>
			</div>
			<?php
			}else echo '<div style="float:left;padding:0px 3px;">'.JText::_('YOU_CANNOT_CREATE_NEW_IDEA').'</div>';
			?>
...
function getNoIdeaFoundText() {
    return '<?php echo JText::_('NO_IDEAS_FOUND'); ?>';
}
