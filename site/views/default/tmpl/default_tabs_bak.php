...
<div id="new_tabs">
	<span style="float: left; padding:10px 10px 0px 10px;"><h3><?php echo ($tab!='top')?'<a href="'.JRoute::_('index.php?option=com_intosuggest&forumId='.$this->output->forum->id).'">':''; ?><?php echo JText::_('TOP'); ?><?php echo ($tab!='top')?'</a>':''; ?></h3></span>
	<span style="float: left; padding:10px 10px 0px 10px;"><h3><?php echo ($tab!='hot')?'<a href="'.JRoute::_('index.php?option=com_intosuggest&forumId='.$this->output->forum->id.'&tab=hot').'">':''; ?><?php echo JText::_('HOT'); ?><?php echo ($tab!='hot')?'</a>':''; ?></h3></span>
	<span style="float: left; padding:10px 10px 0px 10px;"><h3><?php echo ($tab!='new')?'<a href="'.JRoute::_('index.php?option=com_intosuggest&forumId='.$this->output->forum->id.'&tab=new').'">':''; ?><?php echo JText::_('NEW'); ?><?php echo ($tab!='new')?'</a>':''; ?></h3></span>
</div>
			...
			<div style="float:left" class="idea_actions">
				...
				<?php 
						...
						(($this->output->permission->edit_idea_o == 1) && ($this->idea_output->user->id == $idea->user_id))) 
					{
						...
						$edit	= '<img src="'.JURI::base().'/components/com_intosuggest/themes/default/images/edit.png" width="16" height="16" onclick="onedit(\''.$idea->id.'\')" />';
					}
				?>
				<?php 
					$delete = '';
					...
					) 
					{
						...
						$delete	= '<img src="'.JURI::base().'/components/com_intosuggest/themes/default/images/remove.png" width="16" height="16" onclick="ondel(\''.$idea->id.'\')" />';
					}
				?>
				<?php 
					if( $edit!='' || $delete != '')
...
<!-- END: IDEA CONTENT -->
		...
		<div>
			...
			?>
					...
					<a class="comment_text" href="<?php echo JRoute::_(IntoSuggestHelperRouter::addItemId('index.php?option=com_intosuggest&controller=comment&idea_id='.$idea->id))?>"> comment(s)</a>
			</div>
