...
function getList() {
	...
	if (controller == 'comment'){
		...
		var url = "index.php?option=com_intosuggest&controller=comment&task=getList&format=raw&idea_id="+idea_id;
		<?php echo $str_new_ajax; ?>
			method: 'post',
			onComplete: function(txt){
		...
		var url="index.php?option=com_intosuggest&controller=comment&format=raw&task=UdelComment&id=";
		<?php echo $str_new_ajax; ?>
			method: 'post',
			onComplete: function(txt){
				//window.parent.document.getElementById('contentComment').innerHTML = txt;
