...
require_once JPATH_SITE.DS.'components'.DS.'com_intosuggest'.DS.'helper'.DS.'router.php';
function intosuggestBuildRoute(&$query)
{
	...
	if( isset( $query['idea_id'] ) && !isset( $query['task'] ) ) {
		...
		if ( isset( $query['forumId'] ) ) {
			...
			unset($query['forumId']);
			$forum_name 	= IntoSuggestHelperRouter::getForumTitle( $query['forumId'], JText::_('Default') );
		} else {
			$forum_obj 	= IntoSuggestHelperRouter::getForumByIdea( $query['idea_id'] );
			if( $forum_obj ) {
		...
		$idea_title 	= '';
		$idea_title 	= IntoSuggestHelperRouter::getIdeaTitle( $query['idea_id'], JText::_('Default') );
		if (!$obIsJ15) { # Joomla 1.6
		...
		if ( isset($query['forumId']) )
		{
			$forum_name 	= IntoSuggestHelperRouter::getForumTitle( $query['forumId'] );
...
function intosuggestParseRoute($segments)
{
