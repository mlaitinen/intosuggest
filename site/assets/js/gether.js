...
function btnBackTopIdeas_click(forumId) {	
	var urlB = "index.php?option=com_intosuggest&task=getTabs&forumId="+forumId+"&format=raw";
...
function ondel(id) {
	...
	if (agress) {
		var url = "index.php?option=com_intosuggest&controller=idea&task=delIdea&id="+id;
		var req = new Ajax(url, {
			method:'post',
			onComplete:function(txt){
