...
class ModelPermission extends JModel {	
		...
		if( !$obIsJ15 ){
			...
			$query = "
				...
				FROM `#__core_acl_aro_groups`
					INNER JOIN `#__intosuggest_permission`
				WHERE `#__core_acl_aro_groups`.`id` = `#__intosuggest_permission`.`group_id` 
				ORDER BY `#__intosuggest_permission`.`group_id` ASC
			;";
		}
	...
	public function updatePermission($_input) {
		...
		$query = "SELECT * FROM `#__intosuggest_permission` WHERE `group_id`=".$_input['group_id'];
		$res = DBase::getObject($query);
		if($res){
			$query = " 			
				UPDATE `#__intosuggest_permission`".
				"SET ".$set.
				"WHERE `group_id` = ".$_input['group_id'].";";
			DBase::querySQL($query);
		} else {
			$query = "INSERT INTO `#__intosuggest_permission` \n($keys_str) \nVALUES($values_str)";
			DBase::querySQL($query);
		}
	}
}
