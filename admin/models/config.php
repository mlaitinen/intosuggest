...
//		$this->output->account = $uservoice;
	...
	*/
	function getDatetimeList(){
		$sql = "
			SELECT * FROM #__intosuggest_datetime_config
		";
	...
	function addDatetime($value, $description, $default = 0, $id = null){
		if(!$id){
			$sql = "
				INSERT INTO #__intosuggest_datetime_config(`value`, `description`, `default`)
				VALUE('".$value."', '".$description."', '".$default."')
			";
		}else{
			$sql = "
				UPDATE #__intosuggest_datetime_config
		...
		$sql = "
				SELECT max(id) as max FROM #__intosuggest_datetime_config
			";		
	...
	function removeDatetime($id){		
		$sql = "
			DELETE FROM #__intosuggest_datetime_config
			WHERE id = {$id}
		";
		return DBase::getObject($sql);			
	}
	...
	function saveGconfig() {
		...
		$db = &JFactory::getDbo();
		foreach ( $keys as $key ) {
			$query = "REPLACE INTO `#__intosuggest_gconfig`(`key`,`value`) VALUES('$key', '".$gconfig[$key]."')";
			$db -> setQuery( $query );
			$db->query();
		}
	}
