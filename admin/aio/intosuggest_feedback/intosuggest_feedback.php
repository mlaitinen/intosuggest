<?php
/**
 * @version		$Id: intosuggest_feedback.php 164 2011-03-12 09:01:56Z thongta $
...
class plgSystemintosuggest_feedback extends JPlugin {
	/**
	 * construct plug-in
	 *
	 ...
	 */
	function plgSystemintosuggest_feedback(& $subject, $config) {
	...
	function onAfterRoute() {
		...
		if($option == 'com_content' && $view == 'article') {
			...
			$query = "
				SELECT * FROM #__intosuggest_forum_article
			...
			if ($row) {
				if($row->forum_id) {
					...
					$img = 'plugins/system/intosuggest_feedback/feedback.png';
					$link = "?option=com_intosuggest&forumId=$row->forum_id";
					$feedback = "";
		...
		# save the settings
			...
			$query = "
				DELETE FROM #__intosuggest_forum_article
			...
			if($forum_id!=0) {
				...
				$query = "
					INSERT INTO #__intosuggest_forum_article
		...
		# embed the slide into the edit form
			...
			$html='<h3 id="forum_id_title" class="title jpane-toggler-down"><span>IntoSuggest Forum</span></h3>';
	...
	function _getListForum()
	{
		...
		$query = "
			SELECT forum_id
			FROM #__intosuggest_forum_article
			WHERE article_id = $article_id
		";
		$db->setQuery( $query );
		$row = $db->loadObject();
