...
var ThemeDesigner = new Class({
	...
	save: function(name){
		...
		Progress.reset({steps:this.css_length+1, onDone: function(){
			var url = 'index.php?option=com_intosuggest&controller=themes&task=saveSchemaFromParts&format=raw&theme='+name;
		...
		Progress.show();		
		var url = 'index.php?option=com_intosuggest&controller=themes&task=saveTheme&format=raw&theme='+name;
		...
		var part = 1;
		for(key in this.css){
			var url = 'index.php?option=com_intosuggest&controller=themes&task=savePartSchema&format=raw&theme='+name;
...
Dialog.ColorDialog = {
	...
	show: function(ret, onDone){
		...
		if($defined(p.background)){
			...
			iframe.src = "http://localhost/joomlatest/administrator/components/com_intosuggest/views/themes/tmpl/colordialog.html";
...
Dialog.FontDialog = {
	...
	show: function(ret, onDone){
		...
		if($defined(p.background)){
			...
			iframe.src = "http://localhost/joomlatest/administrator/components/com_intosuggest/views/themes/tmpl/fontdialog.html";
			iframe.inject(p.container)
			p.container.inject(ret)
			p.container.setStyle("margin", "0px auto")
			p.background.inject(ret)
